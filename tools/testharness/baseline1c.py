...
def set_ini(pairs):
    ...
    for i, line in enumerate(lines):
        ...
        seen.setdefault((section, key), []).append(i)
    ...
    # docs §83, restored: a key that appears TWICE in one section is silently unsettable. This
    # writer only ever rewrites the FIRST occurrence, and the game reads its ini through
    # GetPrivateProfileString, which also returns the first match - so the two agree today, but
    # nothing enforces that and a duplicate means the file no longer says one thing. Found live:
    # susp_reference_hz was present twice in [jolt_harness]. Both copies read 60 so nothing
    # measured was corrupted, but an A/B on that key would have compared a config against itself
    # and reported a clean null. Refuse rather than write into an ambiguous file.
    ambiguous = {k: v for k, v in seen.items() if len(v) > 1 and k in pairs}
    if ambiguous:
        raise RuntimeError("ini key(s) appear more than once in their section, refusing to write "
                           "an ambiguous file: %r" % (ambiguous,))
