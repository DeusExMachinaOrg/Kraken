...
# THIS DICT PINS EVERY KEY IT LISTS, which means it SHADOWS the compiled-in default. Adding a key
# here is taking responsibility for its value forever. That is not a style note: docs §87/§89
# record a session where the code default of wm4_resync_reseat was flipped to 1 and this dict was
# left at 0, so every batch run through the harness silently measured the un-fixed build - a whole
# four-level sweep came back carrying the §87 pedestal signature and had to be thrown away.
#
# POST-LOSS STATUS. This copy is provably STALE: it has no wm4_resync_period, wm4_resync_reseat,
# wm4_resync_keep_gear or susp_reference_hz, so it predates the §82 divergence instrument
# entirely. It also pins wm4_hardcore, wm4_max_dt, wm4_spin, wm4_steer and wm4_max_substeps, and
# set_ini RAISES on a key the ini does not contain - so on the current branch, where only
# wm4_compress_fraction, wm4_joint_at_mount and wm4_chassis_mass_excl_wheels exist so far, this
# dict fails fast rather than measuring the wrong thing. That is the desired failure: fix the list
# as each lever is re-implemented, do not delete entries to make it run.
BASE_INI = {
    ("jolt", "enabled"): 1,
