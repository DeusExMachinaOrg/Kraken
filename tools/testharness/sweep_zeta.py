...
# docs: this file is TOP-LEVEL executable code with no main() and no guard - importing it
# LAUNCHES THE GAME and runs a full sweep. That is not hypothetical: it happened during the
# post-loss harness repoint, when an import smoke-test over every script started real runs
# and left kraken.ini in test state. Restructuring 62 lines of a stale recovered copy is a
# worse trade than refusing the import outright, so it refuses.
if __name__ != "__main__":
    raise ImportError("sweep_zeta.py runs a live sweep at import time - execute it, do not import it")
