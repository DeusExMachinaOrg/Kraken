...
inline constexpr std::uint16_t kInputCommandWireVersion=3;
inline constexpr std::size_t kInputCommandWireSize=36;
struct InputCommand { std::uint32_t entity_id=0,sequence=0,client_tick=0; float throttle=0,steer=0,brake=0; bool handbrake=false; bool request_unstuck=false; bool horn=false; };
enum class InputCommandCodecError:std::uint8_t { None,OutputTooSmall,InputSizeMismatch,BadMagic,BadVersion,BadFlags,InvalidEntityId,InvalidHandbrake,InvalidUnstuckRequest,InvalidHorn,NonFiniteValue,ValueOutOfRange };
