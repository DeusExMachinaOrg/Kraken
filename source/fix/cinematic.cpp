...
namespace kraken::fix::cinematic {
    ...
    CUSTOM bool CinematicFade(hta::CMiracle3d* self) // custom reimplementation
    {
        ...
        case hta::m3d::CinematicState::CINEMATIC_ENTER_FADE_OUT: {
            ...
            if ((m_cinematic->m_curItem.m_flags >> 1) & 1) {
                ...
                if (cinemaFadePanel) {
                    if (cinemaFadePanel->IsDirectChild(app))
        ...
        case hta::m3d::CinematicState::CINEMATIC_EXIT_FADE_OUT: {
            ...
            // deprecated, fog of war is irelevant for HTA, legacy out of strategy games
    ...
    int32_t __fastcall n_SetCinematicFadeParams(hta::m3d::sArgStack& scriptStack, void* _) {
        ...
        // Would be super nice to have 3 as "both on", but for compatibility reasons this is prefered
        switch (start_mode) {
    ...
    void __fastcall _StartCinematic(hta::ai::Town* self, void* _, hta::ai::Vehicle* pVehicle, const vc3::vector<hta::CVector>& points) {
        ...
        hta::ai::ProcessManager::Instance()->PostMessageA(hta::ai::eGameEvent::GE_SUBSCRIBE,    // eventId
        ...
        hta::ai::ProcessManager::Instance()->PostMessageA(hta::ai::eGameEvent::GE_SUBSCRIBE, hta::ai::Player::Instance()->m_objId, self->m_objId, 0.0f, p2, hta::m3d::AIParam{}, 1);
    ...
    void __fastcall _SetSubmarineState(hta::ai::Submarine* self, void* _, hta::ai::eSubmarineState newState) {
        ...
        if (newState == hta::ai::eSubmarineState::SUBMARINE_MOVES) {
            ...
            hta::ai::ProcessManager::Instance()->PostMessageA(hta::ai::eGameEvent::GE_SUBSCRIBE, hta::ai::Player::Instance()->m_objId, self->m_objId, 0.0f, p1, hta::m3d::AIParam{}, 1);
            ...
            const vc3::vector<hta::CVector2>& vehiclePoints = self->m_entryPath.GetVehiclePoints();
            ...
            if (!vehiclePoints.empty()) {
                ...
                hta::CVector2 intermediatePos{self->m_portPosition.x + self->m_moveDirection.x * 30.0f, self->m_portPosition.z + self->m_moveDirection.z * 30.0f};
            ...
            hta::ai::ProcessManager::Instance()->PostMessageA(hta::ai::eGameEvent::GE_SUBSCRIBE, vehicle->m_objId, self->m_objId, 0.0f, p, hta::m3d::AIParam{}, 1);
        }
