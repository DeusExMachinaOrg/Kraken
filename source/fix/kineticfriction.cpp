...
namespace kraken::fix::kineticfriction {
    ...
    inline float calculateKappa(float wheelRadius, float omega_parallel, float V_parallel, float eps = 0.05f) {
    ...
    inline hta::CVector _CalculateTracePosition(dContact* contact) {
        hta::CVector normal(contact->geom.normal[0], contact->geom.normal[1], contact->geom.normal[2]);
        hta::CVector pos(contact->geom.pos[0], contact->geom.pos[1], contact->geom.pos[2]);
        return pos + (normal * 0.01f);
    }

    int __fastcall CollideWheelSurface(
        hta::ai::Wheel* wheel, int32_t soil_id, float friction, dContact* contact, uint32_t* count, bool reverse
    ) {
        ...
        const float* angVel = dBodyGetAngularVel(wheel->m_body->_id);
        hta::CVector wheel_angular(angVel[0], angVel[1], angVel[2]);
    ...
    int __fastcall CollideWheelAndAsphalt(
        hta::ai::Wheel* wheel, hta::m3d::Object* object, dContact* contact, uint32_t* count, bool reverse
    ) {
    ...
    int __fastcall CollideWheelAndLandscape(
        hta::ai::Wheel* wheel, hta::m3d::Object* ground, dContact* contact, uint32_t* count, bool reverse
    ) {
