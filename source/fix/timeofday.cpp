...
namespace kraken::fix::timeofday {
    ...
    // Reimplements CWorld::UpdateSun (0x005C7760): derive the sun direction from the current
    // time-of-day ascension. Ported from the pre-migration branch -- hta:: namespaces + .hpp headers.
    // The original decompile's game-time / id-list block was dropped: its results were never used and
    // it relied on stable_size_vector, which the migrated headers don't expose.
    void __fastcall UpdateSun(m3d::CWorld* world, void* _) {
        m3d::GlobalTimeParams m_curDayTime   = world->m_weatherManager.m_curDayTime;
        ...
        float sunDirMagnitude = sqrt(world->m_sunDir.x * world->m_sunDir.x +
                                     world->m_sunDir.y * world->m_sunDir.y +
                                     world->m_sunDir.z * world->m_sunDir.z);

        // Normalize the sun direction vector
        world->m_sunDir.x /= sunDirMagnitude;
        world->m_sunDir.y /= sunDirMagnitude;
        world->m_sunDir.z /= sunDirMagnitude;
    }
